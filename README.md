...
GitHub Actions runs the Go workflow checks for pull requests opened from any branch (not only the default branch). Checks also run for merge groups.

If checks fail, open the run details in GitHub Actions to see the failing step output, then update the PR until checks pass.
