...
The skeleton includes multiple agent template files. In commit `f2ab442683ad28f673061c3459309240ea7ee2e5`, these templates are adjusted to normalize how agent “mode” and tool permissions are expressed.
...
Several skeleton agent templates previously listed individual tool permissions as separate entries (for example: `edit: allow`, `bash: allow`, `skill: allow`, `webfetch: allow`). In this commit, many templates remove these per-tool `allow` entries.
