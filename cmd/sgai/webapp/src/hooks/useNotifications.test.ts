...
describe("useNotifications", () => {
  ...
  it("navigates to workspace and closes notification on click", async () => {
    listSpy.mockResolvedValueOnce(
      makeWorkspacesResponse([{ name: "project-a", needsInput: false }]),
    );
    mockSSEEvent = { ts: 1 };
    ...
    Object.defineProperty(window, "location", {
      value: {
        ...
        set pathname(val: string) {
          capturedPathname = val;
        },
        get pathname() {
          return capturedPathname;
        },
      },
      writable: true,
      configurable: true,
    ...
    expect(capturedPathname).toBe("/workspaces/project-a");
