...
#### 1a. Read `state.json` FIRST (highest priority)
...
- [ ] **Read `state.json` FIRST** — This is the single richest signal source. It contains:
  ...
  - **If `state.json` is missing or unreadable, STOP and report this in your analysis log — do NOT proceed to Step 2 without acknowledging this gap**
...
2. **Visit count summary**: From state.json — which agents ran, how many visits each
3. **Message count summary**: From state.json — total inter-agent messages, notable message patterns
...
```
Analysis Summary:
- Files: Read 15/15 session JSONs, state.json, GOAL.md, PROJECT_MANAGEMENT.md, stdout.log, stderr.log
...
#### Efficiency Signals
- [ ] **Visit counts** — From `state.json`, how many times was each agent visited? High counts suggest rework or unclear instructions.
...
1. You have read `state.json` and recorded visit counts and message counts in your analysis log
...
sgai_send_message({
  toAgent: "coordinator",
  body: "RETRO_COMPLETE: No actionable improvements identified for this session. Analysis summary: Read X/Y session JSONs, state.json showed Z agent visits and W messages. Per-category findings: [brief summary of each category observation from Step 1.5]."
})
...
- "I've read enough to conclude there are no suggestions" — NO, unless you have met ALL prerequisites for the No Suggestions Case (state.json read, 3+ session JSONs read, analysis log complete with all 4 categories).

## Checklist

Before marking done, verify:

- [ ] Read `state.json` FIRST and recorded visit counts + message counts
...
- [ ] Read all session artifacts (GOAL.md, PM, state.json, session JSONs, stdout.log, stderr.log)
