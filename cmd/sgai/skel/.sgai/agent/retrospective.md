...
1. **`state.json`** — Contains visit counts, inter-agent messages, and agent sequence. This is the single richest signal source. You MUST read this file.
...
**You may NOT send `RETRO_COMPLETE` or `RETRO_QUESTION` until you have read state.json and at least 3 session JSON files.**
...
- DON'T: Send RETRO_COMPLETE without having read state.json
- DON'T: Base your "no suggestions" conclusion on GOAL.md and PROJECT_MANAGEMENT.md alone
- DO INSTEAD: state.json contains inter-agent messages, visit counts, and agent sequence — these are the primary signal sources for retrospective analysis. You MUST read state.json before drawing ANY conclusions.
...
1. **Discover Artifacts** — Find and read the retrospective session directory (state.json FIRST, then ALL session JSONs)
...
├── state.json                        # Copy of workflow state at session end
...
When you have:
1. Read and analyzed all artifacts (state.json first, then all session JSONs)
