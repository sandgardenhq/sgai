...
func TestHandleAPIMergeWorkspace(t *testing.T) {
	t.Run("rejectsNonRoot", func(t *testing.T) {
		_, forkPath, srv := setupM6ForkWorkspace(t)
		forkName := filepath.Base(forkPath)
		...
		body := strings.NewReader(`{"forkDir":"/some/dir","confirm":true}`)
		req := httptest.NewRequest(http.MethodPost, "/api/v1/workspaces/"+forkName+"/merge", body)
		req.Header.Set("Content-Type", "application/json")
		resp := httptest.NewRecorder()
		mux.ServeHTTP(resp, req)
	...
	t.Run("rejectsMissingConfirm", func(t *testing.T) {
		...
		body := strings.NewReader(`{"forkDir":"/some/dir","confirm":false}`)
		req := httptest.NewRequest(http.MethodPost, "/api/v1/workspaces/root-workspace/merge", body)
		req.Header.Set("Content-Type", "application/json")
		resp := httptest.NewRecorder()
		mux.ServeHTTP(resp, req)
	...
	t.Run("workspaceNotFound", func(t *testing.T) {
		...
		body := strings.NewReader(`{"forkDir":"/some/dir","confirm":true}`)
		req := httptest.NewRequest(http.MethodPost, "/api/v1/workspaces/nonexistent/merge", body)
		req.Header.Set("Content-Type", "application/json")
		resp := httptest.NewRecorder()
		mux.ServeHTTP(resp, req)
